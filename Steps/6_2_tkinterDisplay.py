...
index, VP_x, VP_y, angle = h.GetFile('TonyRoadBig3_Record.txt')
...
for point in turning_point:
    angle[point-20:point] = np.pi/180*45 / 30 * np.sign(angle[point+5])
...
for i, ang in enumerate(angle):
    ...
    current_ang += ang
    if(i >= 400 and i <= 1000):
        u = step * math.sin(current_ang) * 1.3
        v = step * math.cos(current_ang) * 1.3
    elif(i >= 1410 and i <= 1460):
        u = step * math.sin(current_ang) * 0.7
        v = step * math.cos(current_ang) * 0.7      
    else:
